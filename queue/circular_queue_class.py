...
class CircularQueueOneSlotEmpty:
    ...
    def dequeue(self): # 삭제 연산
        if not self.is_empty(): # 공백 상태가 아닌 경우
            self.front = (self.front + 1) % self.N # front 하나 증가 (시계 방향)
    ...
    def peek(self): # 원형 큐의 맨 앞의 요소를 조회
...
def test_basic():
    ...
    # 다시 1개 넣기 -> 선형 큐의 포화 상태처럼 새 요소 추가 불가
    q.enqueue(777)
    q.display()
    print("peek:", q.peek())
    print()
